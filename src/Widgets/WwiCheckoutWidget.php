...
class WwiCheckoutWidget extends Widget
{
    ...
    public function render(array $config = []): string
    {
        ...
        $html .= '<div><label data-editable="name_label" style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);display:block;margin-bottom:6px">' . $this->esc($c['name_label']) . '</label><input class="input" id="wwi-co-name" placeholder="Tu nombre"/></div>';
        $html .= '<div><label data-editable="email_label" style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);display:block;margin-bottom:6px">' . $this->esc($c['email_label']) . '</label><input class="input" id="wwi-co-email" type="email" placeholder="tu@email.com"/></div>';
        $html .= '</div>';
        $html .= '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">';
        $html .= '<div><label data-editable="whatsapp_label" style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);display:block;margin-bottom:6px">' . $this->esc($c['whatsapp_label']) . '</label><input class="input" id="wwi-co-phone" placeholder="+57 300 000 0000"/></div>';
        $html .= '<div><label data-editable="domain_label" style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);display:block;margin-bottom:6px">' . $this->esc($c['domain_label']) . '</label><input class="input" id="wwi-co-domain" placeholder="tunegocio.com"/></div>';
        $html .= '</div>';
