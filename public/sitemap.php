...
try {
    ...
    $pages = $db->query("SELECT slug, updated_at FROM pages WHERE site_id = 1 AND status = 'published' AND no_index = 0 ORDER BY sort_order ASC")->fetchAll();
    foreach ($pages as $p) {
    ...
    $posts = $db->query("SELECT slug, updated_at FROM blog_posts WHERE site_id = 1 AND status = 'published' ORDER BY published_at DESC")->fetchAll();
    foreach ($posts as $p) {
        $lastmod = date('Y-m-d', strtotime($p['updated_at']));
