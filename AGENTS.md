...
- **Design System (sep 2026)**: ver `WWI-DESIGN-SYSTEM-MASTER-PROMPT.md` (roles, tokens, motion system, presupuesto de performance, WCAG 2.2 AA, 55 innovaciones, gobernanza). Primer lote implementado: landing wwi con spotlight de cursor con inercia, barra de progreso de scroll y reveals nativos (`animation-timeline: view()`, fallback IntersectionObserver, reduced-motion); admin con **Ctrl+K command palette** (paneles + acciones Factory/BRICK, búsqueda difusa), **View Transitions** entre paneles (`W.vt` + `#wontia-app{view-transition-name}` + animación de entrada vía MutationObserver), **badge de jobs en vivo** en topbar (poll 60s, solo superadmin) y **modo densidad** compacta/cómoda (`wwi_density` en localStorage). Pendiente: hero WebGPU (fase 2), contadores animados, toggle de tema con transición radial.
...
- **GIT SELF-UPDATE (sep 2026)**: `POST /api/v1/admin/system/update` (superadmin, panel Factory → System) escribe solicitud firmada (HMAC `system_update|ts`) en la cola; el agente host `/root/wwi-update.py` (cron `* * * * *`, `deploy/wwi-update.py`) hace git clone del repo público → rsync (preserva .env/vendor/cache/uploads) → docker build → recrea TODOS los contenedores `wontia-web-intelligence` capturando su config vía docker inspect → health-check de cada uno → **rollback automático** a `:previous` si falla. E2E verificado 2 veces (6 contenedores, ~20s). El deploy agent ignora `update-*.json` y viceversa. **Lockfile `flock` (sep 2026)**: ambos agentes toman `.update.lock`/`.deploy.lock` — evita runs concurrentes cuando un update tarda >60s (race real detectada: recreates conflictivos + rollback falso; corregido). Status con escritura atómica (`.tmp` + `os.replace`).
...
- **HERO v2 (sep 2026)**: `WwiHeroWidget` rediseñado (self-contained, CSS+JS propios `iqh-*`): contenedor 1200px a 2 columnas (copy | visual), eyebrow con dot vivo, H1 con gradiente, **prompt IA en el hero** (`wwiHeroPrompt()` → abre wizard y envía el prompt), chips de sector que cambian el mockup (`data-mock`), marco de navegador con shimmer + barra "TIA construyendo" con % animado, badges flotantes (SSL/dominio/24h), prueba social (avatares + 4.9), stats glass con contadores, marquee contenido, orbes+grid de fondo, tilt 3D, responsive y reduced-motion. El tema ya no inyecta el panel TIA en `wwi-hero` (`wwi-iq-hero-solo`) para evitar duplicidad. Copy editable en Pages → home → Sections (id 39). **Mini-builder interactivo (sep 2026)**: el mockup es un builder jugable (`.iqb`): 10 bricks arrastrables, agregar/eliminar/duplicar/ocultar/reordenar secciones, undo/redo/reset, plantillas rápidas, device switcher, score en vivo, persistencia local y "Usar esta estructura" → abre el wizard con el prompt generado.
- **WWI INLINE BUILDER (sep 2026)**: constructor visual real por filas/columnas/slots/bloques. Spec: `WWI-INLINE-BUILDER-MASTER-PROMPT.md`. Schema `install/builder.sql` (6 tablas: `wwi_page_rows/columns/slots/blocks/revisions/trash`, multi-tenant). `BuilderService` (árbol, CRUD, reorder, duplicar, mover, convertir secciones→filas con snapshot, revisiones, papelera, render con `data-row/col/slot/block` y CSS base `wwi-b-*`). API `/api/v1/admin/builder/*` (tree, palette, rows, columns, blocks, reorder, convert, publish, revisions, trash, render). Editor `public/assets/js/builder.js` + `builder.css` (inyectados por el live-editor cuando hay JWT): barra "🧱 Bloques", convertir a filas, decoración con etiquetas de fila/columna, **"+" flat 64px cuadrado redondeado borde discontinuo** en slots vacíos, selección con toolbar flotante (mover/duplicar/subir/bajar/eliminar), **borrar → aparece "+" en su lugar** con Deshacer, panel contextual por tipo (rich text para texto, URL/alt/caption para imagen, label/href/estilo para botón, props JSON para bricks), paleta con buscador (6 elementos + 43 bricks), versiones con restaurar. **Drag & drop**: bloques entre slots (línea de inserción) y **filas completas** arrastrando la etiqueta "Fila N" (reorder/row persistido). Render del árbol en temas `default` y `wwi-intelligence`. Verificado E2E en `cafeteria-quindio.wontia.com/galeria`.
- **HERO DEMO COMERCIAL (sep 2026)**: el mockup del hero es ahora **`wwi-builder-demo`** (brick propio y movible: "WWI: Demo interactiva"). Modo **vista** por defecto (solo sitio de ejemplo + badge "Sitio de ejemplo · creado con TIA") con pill **✨ Personalizar / ▶ Tour**; el chrome técnico (paleta, barra, score) solo aparece en modo edición. El demo es un **mini-sitio real con contenido** por sector (4 packs: restaurante/abogado/tienda/gimnasio + genérico): hero con **foto**, servicios con tarjetas, galería, video, precios, testimonios, FAQ desplegable, contacto, CTA y footer; **mini-nav interna con scroll suave**; botones abren el wizard con el prompt del sector; clic en brick existente resalta+scroll (Shift duplica). **Tour automático (attract loop)**: 9 pasos animados con captions, arranca solo al entrar en pantalla, pausa al hover, se detiene al interactuar o con el botón, respeta reduced-motion y restaura el estado.
- **Domain Checker REAL (RDAP + Porkbun + DNS)**: `DomainCheckerService` — (1) **Porkbun API autoritativa** si `wwi.porkbun_api_key`/`wwi.porkbun_secret_key` están en Factory → Config (llaves cargadas sep 2026; secret enmascarada en el panel); soporta .com/.net/.org/.co/.site/.info/.store/.online/.xyz/.shop — NO soporta `.com.co` (INVALID_TLD); (2) RDAP (Verisign .com/.net, IANA bootstrap) cuando no hay registrador; (3) fallback **DNS** para TLDs sin RDAP (`.co`/`.com.co`): DNS activo → TAKEN, sin DNS → CHECKING "parece libre". Caché 5 min, ERROR 60s. Verificado: sonika.com/co → TAKEN con precio real; dominio inventado → AVAILABLE.
- **Domain Cost Engine**: `wwi.domain_costs` (settings JSON por TLD: reg/ren USD) — **actualizado sep 2026 con precios REALES de Porkbun** (com 11.08, net 12.52, org 7.98/11.84, co 15.76/31.20, site 1.96/28.84, info 3.60/22.14, store 2.57/43.77, online 1.96/28.84, xyz 2.04/14.21, shop 2.06/31.41) editables en Factory → Config; Margin Guard usa `wwi.domain_cost_usd`. Reproducir con `POST https://api.porkbun.com/api/json/v3/pricing/get`.
...
- **WWI STORE — Ecommerce por tenant (Fase 3, sep 2026)**: módulo completo de tienda. Schema `install/store.sql` (7 tablas: `store_products`/`store_variants`/`store_categories`/`store_customers`/`store_orders`/`store_order_items`/`store_shipping_zones`, todas con `site_id`; migraciones idempotentes `ip_hash`/`stock_released` en `StoreService::migrate()`). `StoreService` (pedido transaccional con re-precio server-side y stock atómico `stock >= :minq`, Wompi firma `SHA256 reference|cents|currency|integrity_key` + webhook fail-closed 401, emails de pedido). API admin `/api/v1/admin/store/*` (overview/products/categories/orders/zones/settings/ensure-tables/maintenance) + pública `/api/v1/public/store/*` (config/products/products/{slug}/categories/zones/orders/orders/{uuid}/payments/wompi/webhook). Widgets `store-catalog`/`store-product`/`store-cart`/`store-checkout` (carrito localStorage `wwi_cart_<site>`, checkout con Wompi Web Checkout, contra entrega y WhatsApp). Panel admin **Tienda** (`#store`) con Productos (selector de imágenes desde Media), Categorías, Pedidos (estados pago/fulfillment), Envíos y Configuración (llaves Wompi propias del tenant, límites anti-spam, mantenimiento). **Plan `ecommerce`** en Factory (COP 899.000 / USD 225, margen 37%) + provisioning crea tienda, settings, zona "Nacional" y página `/tienda`. **Anti-spam**: máx 8 pedidos/hora por IP y 4 por correo (settings `store_max_orders_hour_ip|email`) → HTTP 429. **Liberar stock**: pedidos `pending` con método ≠ `cod` y >60 min → `releaseExpiredOrders()` restaura stock y marca failed/cancelled (cron `job-runner.php` global + `POST /store/maintenance` por tenant). **SMTP por tenant**: `EmailService` resuelve env → settings `mail_*`; Factory → Config (`wwi.mail_*`) + botón "Sincronizar correo a todos los sitios" (`POST /factory/mail/sync`) + copia automática en provisioning (`copyMailConfig`). E2E verificado: pedido COD → stock 10→8, rate limit 429 al 5º, liberación 3→4, mail sync 8 sitios, tenant nuevo `tienda-e2e.wontia.com` con tienda viva.
