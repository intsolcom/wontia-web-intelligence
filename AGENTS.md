...
- **Plantillas por sector (Fase 2)**: `install/wwi_templates_seed.sql` — 20 plantillas (1 por categoría) con preset JSON (secciones/widgets del tema default + color de marca). API pública `/api/v1/public/templates` ahora devuelve `{categories, templates}` y la landing las muestra. TIA las usará como base del Site Generator (Fase 2 siguiente).
