...
- **Preview RICO (sep 2026)**: el generador sigue el spec `WWI-UNIVERSAL-WEBSITE-MASTER-PROMPT.md` — estructura FIJA de plantilla básica (header·hero·valor·servicios·about·beneficios·testimonios·CTA·contacto·footer) con contrato JSON estricto (colors primary/secondary, placeholders {{CAMPO}}, copy conversión). El render del preview usa design system profesional con tokens del sector. X-Frame-Options = SAMEORIGIN (el modal del preview es iframe).
