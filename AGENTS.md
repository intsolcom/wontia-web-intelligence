...
- **WWI FACTORY — dominio propio**: tenant dedicado en contenedor `wontia-wwi` (puerto 4009, `SITE_ID=3`, `APP_URL=https://wwi.wontia.com`, BRICK_API_KEY propia). Vhost host nginx `/etc/nginx/sites-enabled/wwi.conf` (80→301, 443 con cert Let's Encrypt propio + HSTS → proxy 4009). DNS en Hostinger: `wwi.wontia.com A 169.58.12.55`. El operador de la Factory usa `https://wwi.wontia.com/admin.php#factory`. Cert renovable con el cron de certbot estándar.
